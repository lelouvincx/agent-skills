...
- Move the local agent-browser workflow into an on-demand Amp convention and require each headed Chrome session to use its persistent local profile on a new, dedicated CDP port, with user-assisted sign-in when authentication is required.
