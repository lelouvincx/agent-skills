...
- Before starting, inspect local files—including ignored files—and move or back up unrelated files outside the synced repository. Diverged non-UTF-8 files, such as images and PDFs, fail automatic merge and remain local; reconcile each reported failure manually.
