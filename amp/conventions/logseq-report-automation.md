...
- `lelouvincx-bot` may access only repositories in the shared `amp/agent-secrets/github-identities.json` policy, with write but not admin permission and no organisation membership.
- Before changing shared agent execution, identity access or publishing functions, resolve the `agent-skills` checkout with `project-resolve agent-skills --json`. Edit shared implementation and account-wide identity policy there. Keep Logseq-specific paths, publishing targets and operation authorization in Logseq's `automation/lib-agent.sh` adapter and deterministic publisher.
