...
The private newline-JSON socket operations are `managed_status`, `managed_initialize` and `managed_shutdown`. Initialization may attempt the exact CDP connection only once. Shutdown requests `Browser.close` before disconnecting and exiting; if the saved CDP socket is already closed, it performs one bounded reconnect to the exact `AGENT_BROWSER_MANAGED_CDP` endpoint and fails closed if that endpoint is unavailable. These operations belong to the lifecycle controller, not the public CLI or MCP tool surface. MCP refuses calls in managed mode. The internal `--managed-build-id` probe returns the native compile-time identity without starting a session; it is distinct from the packaging fingerprint.
