...
status: "active"
summary: "Registers an experimental Amp high-mode agent that uses Claude Fable 5.1."
artifact:
  id: "claude-fable-5-1"
  type: "agent_mode"
  surface: "mode_picker"
...
source:
  kind: "plugin"
  file: "plugins/claude-fable-5-1.ts"
  scope: "system"
...
contract:
  ...
  event: null
  command_id: null
  agent_mode_key: "claude-fable-5-1"
...
runtime:
  ...
  dependencies:
    ...
    - "anthropic/claude-fable-5-1 model availability"
  env: []
  reads:
  ...
  network:
    - "Anthropic Claude Fable 5.1 through Amp"
...
`claude-fable-5-1` adds Claude Fable 5.1 to Amp's mode picker. It keeps Amp high mode's prompt, tools, and reasoning effort, and changes only the model.

## Invocation

- Surface: Amp mode picker
- Registered with: `amp.experimental.registerAgentMode`
- Mode key: `claude-fable-5-1`
- Label: `Claude Fable 5.1`
- Plugin file: `plugins/claude-fable-5-1.ts`

## Contract

The mode uses `anthropic/claude-fable-5-1` and the `Claude Fable 5.1` display label. It extends Amp high mode, so it inherits that mode's system prompt, tool list, and reasoning effort. It accepts a normal user prompt and starts an agent thread.
...
Select `Claude Fable 5.1` when starting a thread, then enter a normal coding request.

## Troubleshooting

- If the mode is missing, check that the plugin loaded and `amp.experimental` is available.
- If the model fails, check that `anthropic/claude-fable-5-1` appears in `amp plugins show-agent-options --json`.

## Maintenance notes

Keep the `@amp-agent-mode` metadata in sync with the registered key and label. Keep the model override in `plugins/claude-fable-5-1.ts`. Do not copy a static high-mode prompt into the plugin; the mode should keep extending Amp high mode.
