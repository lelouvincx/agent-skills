...
The tool is registered only in a process with the exact opt-in value `1`. Deployment must set this only in the configured stable-runner process. The Plugin API does not expose a runner ID. The tool therefore proves process eligibility through invocation and records `ctx.thread.id` without making a runner-ID query.
...
This tool reads and may insert one row in the local `recipients` table. The enabled plugin may create the Amp state directory, database file and 2 ownership tables before it registers tools. The tool makes no network request, starts no process, changes no thread and reads no secret.
