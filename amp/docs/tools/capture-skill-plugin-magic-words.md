...
A passive `tool.result` handler captures terminal invocations for the first reliable tracked tool set: `claude_code_subagent` and `pi_code_subagent`. It records the tool name, terminal status, compact summary, artifact metadata, and privacy booleans. It must not store raw tool input, raw output, error text, prompts, transcripts, or command payloads. Dataset-maintenance capabilities such as `track_event` and `label_skill_plugin_usage` are excluded from self-capture.
