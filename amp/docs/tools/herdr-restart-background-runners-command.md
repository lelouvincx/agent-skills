...
status: "active"
summary: "Creates a worker Amp thread that restarts background Herdr Amp runner tabs except agent-skills."
artifact:
...
safety:
  ...
  user_gate: "manual command palette invocation and confirmation dialog"
  constraints:
    ...
    - "The worker prompt excludes the agent-skills runner."
...
1. Use `herdr workspace list`, `herdr agent list`, and `herdr tab list --workspace <background_workspace_id>`.
2. Find the workspace labelled exactly `background`.
3. Select only Amp agents in that workspace whose `cwd` basename is not `agent-skills`.
4. Use runner IDs in the form `macbook.<directory>`.
5. Use this restart command for each runner:
...
If Chinh confirms, the command creates a visible worker thread with `show: true`. It uses the active thread ID as `parentThreadID` when available. It then appends the workflow prompt to the worker.
...
The command creates a new Amp thread and appends a user message to it. It can change Amp focus because it creates the worker with `show: true`.
